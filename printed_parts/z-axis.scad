...
module zrodholder2(thickness=(i_am_box == 0 ? 14 : 15), bottom_thickness=5, ylen=44, xlen=34, zdelta=0){
    holder_inner_r = 9;
    holder_inner_r2 = 2;
    difference(){
        union(){
            difference(){
                union(){
                    cube_fillet([5, ylen, thickness], vertical=[3, 3, 0, 0], top = [thickness / 1.7, 0, 0, 5]);
                    //hole for Z axis is thru this
                    cube_fillet([xlen, ylen, bottom_thickness], vertical=[3, 3, 0, 3]);
                    translate([5, 5, 0]) {
                        difference(){
                            cube([holder_inner_r2, holder_inner_r2, thickness - 5.5]);
                            translate([holder_inner_r2, holder_inner_r2, -0.5])
                ...
                translate([board_to_xz_distance - z_delta, 9, -1]) cylinder(h=board_thickness+20, r=bushing_z[0] + single_wall_width / 4);
					 // bearing hole
					 translate([board_to_xz_distance - z_delta, 26, -1]) {
                	cylinder(h=(2 * bottom_thickness / 3) + 1, r=11.25);
						cylinder(h=bottom_thickness + 2, r=5);
					 }
                //inside rouned corner
                translate([0, 5, -1]) cylinder(r=0.8, h=100, $fn=8);
                //side screw
